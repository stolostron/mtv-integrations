...
3. **Provider Secret** — in the `mtv-integrations` namespace; contains kubeconfig token, CA cert, and a `cacert` key for MTV compatibility. Extra PEM certificates in `cacert` (custom CAs) are preserved; the MSA `ca.crt` is appended if missing. `token` continues to track MSA token rotation. Operator steps: [Custom CA for MTV provider secrets](CUSTOM_CA.md).
