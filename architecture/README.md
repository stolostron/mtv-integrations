...
- **Creating and managing resources:**
  ...
  - **Provider Secret:**  
    The secret is created by the ManagedServiceAccount controller from the ManagedServiceAccount resource, containing the kubeconfig connectivity token and CA certificate for a managed cluster. For compatibility with the MTV provider, the `ca.crt` value is also duplicated under the `cacert` key in the secret, which is placed in the central MTV namespace (typically `openshift-mtv`). Additional PEM certificates already present in `cacert` (custom CAs) are preserved; the controller appends the current MSA `ca.crt` only if it is missing from the bundle. See [Custom CA for MTV provider secrets](../docs/CUSTOM_CA.md).
  - **Provider Resource:**  
