...
#GOTOOLCHAIN_VERSION pins the Go toolchain to match the toolchain directive in go.mod, avoiding
# version-mismatch errors when go test is run in a pipeline (where GOTOOLCHAIN=auto re-exec may not fire).
GOTOOLCHAIN_VERSION ?= $(shell grep '^toolchain ' go.mod | awk '{print $$2}')
